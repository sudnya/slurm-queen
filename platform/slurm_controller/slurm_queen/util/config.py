...
def setup_config(dictionary = {}):
    return config.ConfigurationSet(
        config.config_from_env(prefix="SLURM_QUEEN", separator="_", lowercase_keys=True),
...
def config_path():
    ...
    home_config_path = os.path.join(home, ".slurm_queen_config.yaml")
    if os.path.exists(home_config_path):
        return home_config_path

    return os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "configs", "slurm_queen_config.yaml")
